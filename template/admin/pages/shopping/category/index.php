    ...
    <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-8 gap-4">
        <div>
            <h1 class="text-2xl font-bold text-gray-800">
                Shopping <?= $shopping->name ?> :: Categorias
            </h1>
        ...
        <a href="<?= url("padmin/shoppings/categories/{$shopping->id}/create") ?>"
            ...
            <?php foreach ($categories as $category): ?>
                <div class="bg-white rounded-xl border border-gray-200 shadow-sm flex flex-col transition-all hover:shadow-lg">
                    <div class="p-5 flex-grow">
                        <h3 class="text-lg font-semibold text-gray-800 truncate" title="<?= $category->name ?>">
                            <?= $category->name ?>
                        </h3>
                    </div>

                    <div class="p-3 bg-gray-50 border-t border-gray-200 rounded-b-xl flex justify-end items-center gap-2">
                        <a href="<?= url("padmin/shoppings/categories/{$shopping->id}/edit/{$category->id}") ?>"
                            class="p-2 text-gray-500 hover:bg-gray-200 hover:text-gray-800 rounded-full transition-colors"
                            title="Editar">
                            <i data-lucide="edit-3" class="w-4 h-4"></i>
                        </a>
                        <a href="<?= url("padmin/shoppings/{$shopping->id}/categories/{$category->id}/items") ?>"
                            class="p-2 text-gray-500 hover:bg-gray-200 hover:text-gray-800 rounded-full transition-colors"
                            title="Itens">
        ...
        <?php else: ?>
            <div class="col-span-1 sm:col-span-2 lg:col-span-3 xl:col-span-4 bg-white rounded-xl border border-dashed border-gray-300 p-12 text-center">
                <div class="max-w-xs mx-auto">
                    ...
                    <h3 class="mt-4 text-lg font-semibold text-gray-800">Nenhuma categoria encontrada</h3>
                    <p class="mt-1 text-sm text-gray-500">
                        Comece adicionando uma nova categoria no shopping.
                    </p>
                    <div class="mt-6">
                        <a href="<?= url("padmin/shoppings/categories/{$shopping->id}/create") ?>"
