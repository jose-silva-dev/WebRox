...
<div class="bg-white rounded-xl border border-neutral-200 shadow-sm p-6 space-y-6">
    ...
    <div class="p-6 border-b border-gray-200">
        <nav aria-label="Breadcrumb">
            <ol class="flex items-center gap-1.5 text-sm text-gray-500">
                ...
                <li>
                    <span class="font-medium text-gray-700">editando <?= $shopping->name ?></span>
                </li>
            </ol>
        </nav>
    ...
    <form action="<?= url("padmin/shoppings/update/{$shopping->id}") ?>" method="POST" class="p-6">
        <div class="space-y-4">
            <div>
            ...
            <div>
                ...
                <input
                    ...
                    class="block w-full px-4 py-2.5 border border-gray-300 rounded-lg shadow-sm placeholder-gray-400 
                                   outline-none focus:ring-2 focus:ring-amber-500 focus:border-amber-500 
                                   transition-shadow sm:text-sm"
                    placeholder="Ex: Shopping de venda de itens full"
                    value="<?= $shopping->description ?>"
                    required>
            </div>
            <div class="grid md:grid-cols-2 gap-5 grid-cols-1">
                <div>
                    <label for="coin" class="block text-sm font-medium text-gray-700 mb-2">
                        Coluna da moeda
                    </label>
                    <input
                        type="text"
                ...
                <div>
                    <label for="coin_name" class="block text-sm font-medium text-gray-700 mb-2">
                        Nome da Moeda
                    </label>
                    <input
                        type="text"
        ...
        <div class="mt-8 flex justify-start">
            <button
                ...
                <span>Salvar Shopping</span>
            </button>
        </div>
    </form>

    <div class="flex justify-end">
        <a href="<?= url("padmin/shoppings/delete/{$shopping->id}") ?>"
            onclick="confirm('Tem certeza que deseja remover o shopping \'<?= $shopping->name ?>\'?')"
