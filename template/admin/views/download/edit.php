...
<div class="space-y-1">
    ...
    <form action="<?= route("admin.download.update.{$download->id}") ?>" method="post" class="form space-y-2">
        ...
        <div>
            ...
            <input type="text" name="description" id="description" value="<?= $download->description ?>" placeholder="Digite a descrição do download" required>
        </div>

        <div>
            <label for="link">URL para Download</label>
            <input type="url" name="link" id="link" value="<?= $download->link ?>" placeholder="Digite o link do download" required>
        </div>

        <div>
            <label for="size">Tamanho</label>
            <input type="text" name="size" id="size" value="<?= $download->size ?>" placeholder="Digite o tamanho do download" required>
        </div>
