...
<div class="space-y-1">
    <form action="<?= route("admin.account.search") ?>" method="post" class="form space-y-2">
        <?= csrf_field() ?>
        <div>
            <label for="account">Buscas Conta</label>
            <input type="text" name="account" id="account" placeholder="Digite o login do usuário" value="<?= $account ?? "" ?>" required>
        </div>
        <div>
            <button type="submit" class="btn btn-danger"><i class="ph ph-magnifying-glass"></i> Buscar</button>
        </div>
    </form>
</div>
...
<?php if ($membInfo): ?>
    <div class="information">
        <div class="details">
            ...
            <?php endforeach; ?>
            <hr>
            <p>Cash: <span><?= $membInfo['cash'] ?></span></p>
        ...
        <div class="action" x-data="{ view: 'data' }">
                ...
                <form action="<?= route("admin.account.update.data.$account") ?>" method="post" class="form space-y-1">
                    ...
                    <div>
                        ...
                        <select name="status" id="status">
                            ...
                            <option value="<?= config('staff_code') ?>" <?= $membInfo['status'] == 2 ? 'selected' : '' ?>>Staff</option>
                        </select>
                    </div>
                    <div>
            ...
            <div data-view="vip" x-show="view === 'vip'">
                <form action="<?= route("admin.account.update.vip.$account") ?>" method="post" class="form space-y-1" x-data="{ vipType: '<?= array_search($membInfo['vip'], resolve('User')->getConfigUser()['vip']['name']) ?>' }">
                    <?= csrf_field() ?>
                    ...
                    <div x-show="vipType != '0'">
                        ...
                        <input type="number" name="amount" id="vip-amount" value="0" min="1" required x-bind:disabled="vipType == '0'">
                    </div>
                    <!-- Campos sempre presentes - valores serão atualizados via JavaScript antes do submit -->
                    <input type="hidden" name="action" id="hidden-action" value="add">
                    <input type="hidden" name="amount" id="hidden-amount" value="0">
                    <div x-show="vipType == '0'" style="color: #999; font-size: 12px; padding: 8px; background: #f5f5f5; border-radius: 4px;">
